...
type TagDescriptorsFn<Result> = <TResult extends Result, TTag extends TagType>(
	result: TResult | undefined,
	tagType: TTag
) => ReadonlyArray<TagDescription<TTag>>

/** Returns the provided tag descriptors for a LIST result. */
function providesList<R extends ResultBase[], T extends TagType>(result: R | undefined, tagType: T) {
	return result
...
function providesDetail<R extends ResultBase, T extends TagType>(result: R, tagType: T) {
	return result ? [{ type: tagType, id: result.id }] : []
}

/** Returns the invalidated tag descriptors for a LIST result . */
function invalidateList<T extends TagType>(tagType: T) {
	return [{ type: tagType, id: 'LIST' as const }]
}

/** Returns the invalidated tag descriptors for a DETAIL result. */
function invalidateById<R extends ResultBase, T extends TagType>({ id }: R, tagType: T) {
 ...
 */
function getTagDescriptors<R extends ResultBase | ResultBase[], T extends TagType>(
	result: R,
	tagType: T,
